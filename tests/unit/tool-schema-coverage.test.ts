...
describe("Tool Schema Coverage", () => {
  ...
  it("enforces period enum and about tool enum constraints", () => {
    ...
    assert.ok(/mode\s*:\s*z\.enum\(\["summary",\s*"tool",\s*"prompts",\s*"resources",\s*"metrics",\s*"routing"\]\)/.test(aboutSchema), "about_mcp_tools should enforce mode enum including 'routing'");
