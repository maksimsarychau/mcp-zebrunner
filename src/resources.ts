...
export function buildMcpRoutingContent(): object {
  return {
    title: "Advanced Zebrunner MCP Server — dual-MCP routing guide",
    summary:
      "When this server (mcp-zebrunner, the Advanced Zebrunner MCP Server) is loaded alongside Zebrunner's official hosted MCP (config key 'zebrunner', URL https://{workspace}.zebrunner.com/api/mcp), use this table to pick the right server for each task.",
    servers: [
      {
        config_key: "zebrunner",
        display_name: "Official Zebrunner MCP (beta)",
        ...
        best_for: [
          ...
          "Issue link / unlink bulk operations on tests",
        ],
      },
      {
        config_key: "mcp-zebrunner",
        display_name: "Advanced Zebrunner MCP Server (this server)",
        tool_prefix: "adv_",
        best_for: [
          "Failure forensics (adv_analyze_test_failure, adv_detailed_analyze_launch_failures)",
          "Reporting / dashboards (adv_generate_report, adv_regression_results_analyzer, adv_generate_weekly_regression_stability_report)",
          "Runtime + flakiness analytics (adv_analyze_regression_runtime, adv_find_flaky_tests)",
          ...
          "Preview/confirm safe mutations (adv_create_test_case, adv_update_test_case, adv_manage_test_run, adv_import_launch_results_to_test_run, adv_rerun_launch_failures, adv_start_launch)",
...
export function registerResources(server: McpServer, deps: ResourceDependencies): void {
  ...
  // ── 1.2b Static: Dual-MCP Routing Guide ────────────────────────────────────

  server.registerResource(
    "mcp_routing",
    "zebrunner://mcp-routing",
    {
      description: "Dual-MCP routing guide explaining when to use the official Zebrunner MCP versus the Advanced Zebrunner MCP Server (this server). Read when both MCPs are connected.",
    },
    async () => ({
      contents: [{
        uri: "zebrunner://mcp-routing",
        mimeType: "application/json",
        text: JSON.stringify(buildMcpRoutingContent(), null, 2),
      }],
    }),
  );
  ...
  debugLog("Registered 14 MCP resources (6 static + 8 templates)");
}
